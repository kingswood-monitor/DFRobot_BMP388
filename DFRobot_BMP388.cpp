...
int8_t user_i2c_write(uint8_t dev_id, uint8_t reg_addr,uint8_t *data, uint16_t len){
  Wire.beginTransmission(dev_id); // Transmit to device #8
  Wire.write(reg_addr);              // Send one byte
  uint8_t i = 0;
  for(i = 0; i < len; i++){
    Wire.write(*data); 
    data++;
...
int8_t user_i2c_read(uint8_t dev_id, uint8_t reg_addr,uint8_t *data, uint16_t len){
  int i = 0;
  Wire.beginTransmission(dev_id); // Start transmission to device 
  Wire.write(reg_addr); // Send register address to read rom
  Wire.endTransmission(); // End transmission
  
  Wire.requestFrom(dev_id, len);// Send data n-bytes read
